...
async function metaGet(path, params = {}) {
  const url = new URL(`https://graph.facebook.com/${META_VER}/${path}`);
...
async function main() {
  ...
  // Estruturas de dados
  // thermo: {assoc: {unit|||cname: {g,l,i,c}}}  ← uma entrada por instância, para o termômetro
  // display: {assoc: {cname: {g,l,i,c}}}         ← agrupado por nome, para a tabela de criativos
  // units:   {assoc: {unit: {g,l}}}              ← por unidade, para a tabela de metas
  const thermo  = {};
  const display = {};
  const units   = {};
  ...
  for (const account of ACCOUNTS) {
    ...
    try {
      ...
      for (const row of rows) {
        ...
        // Thermo — instância separada (unit × criativo)
        ...
        // Display — agrupado por nome de criativo
